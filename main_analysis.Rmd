...
setwd("~/Documents/GitHub/influenza_A_cross-reactivity")
...
```{r ed_fig_1, echo=FALSE, message=FALSE, warning=FALSE, fig.width=26, fig.height=20}}

# Extended Data Figure 1

ferret_data = lapply(c(1,2,4), function(i){
  
  if(i != 4){
    
    data_used = dataList[[i]] %>%
      mutate(
        study = names(dataList)[i],
        log_titer = floor(log_titer)
      ) %>%
      select(
        study,
...
panelList = lapply(yrs, function(i) rawDataByYear(i, ferret_data))
